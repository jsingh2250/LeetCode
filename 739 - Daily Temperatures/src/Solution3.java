...
class Solution {
    public int[] dailyTemperatures(int[] temperatures) {
        ...
        for (int currentDay = temperatures.length - 1; currentDay >= 0; currentDay--) {
            int temperatureOfCurrentDay = temperatures[currentDay];
            if (temperatureOfCurrentDay >= hottestTemperature) {
                hottestTemperature = temperatureOfCurrentDay;
                continue;
            }

            int numberOfDaysUntilWarmerTemperature = 1;
            while (temperatures[currentDay + numberOfDaysUntilWarmerTemperature] <= temperatureOfCurrentDay) {
